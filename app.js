...
app.post("/search/result",function(req,res){
	...
	Job.find({},function(err,jobs){
		...
		else
		{
			...
			nearby = zipcodes.near(pin, Number(km)*1000, { datafile: 'zipcodes1.csv' });
			nearby.then(function(result){
				console.log(result);
				console.log(km*1000);
				jobs.forEach(x => {
                 if(result.includes(x.pincode)==true){
				...
				res.render('search.ejs',{jobs:jsend,currentUser:req.user});
			});
		...
		Job.find({},function(err,jobs){
			...
			else
			{
				...
				nearby.then(function(result){
					...
					res.render('mjob.ejs',{jobs:jsend});
				});
